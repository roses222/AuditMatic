...
def detect_header_row_index(path: str, format_type: str) -> int:
    ...
    if format_type == 'excel':
        ...
        try:
            ...
            if ws is None:
                return 0
            fallback_software_idx = None
            for idx, row in enumerate(ws.iter_rows(values_only=True), 0):
                headers = [normalize_header(cell) for cell in row if cell]
                has_software = any('SOFTWARE' in h for h in headers) or any('COMPONENT' in h for h in headers)
                has_current_ci = any('CURRENT CI VERSION' in h for h in headers)
                has_version_locations = any('VERSION LOCATION' in h or 'VERSION LOCATIONS' in h for h in headers)

                # Prefer rows that look like true workbook headers.
                if has_software and (has_current_ci or has_version_locations):
                    return idx
                if has_software and fallback_software_idx is None:
                    fallback_software_idx = idx
            if fallback_software_idx is not None:
                return fallback_software_idx
        except Exception:
            pass
        return 0
...
def _create_mock_registry_snapshot() -> Dict[str, Any]:
    """Generate a mock registry snapshot payload."""
    return {
...
def _create_mock_checklist_json() -> Dict[str, Any]:
    """Generate a mock checklist payload."""
    return {
        ...
        "checklist_name": "mock_checklist",
        "generated_at": "2026-04-17T14:32:00",
        "baseline_version": "BASELINE_MOCK_v3.2",
...
def _create_mock_result_json() -> Dict[str, Any]:
    """Generate a mock audit result payload."""
    return {
        ...
        "audit_workbook": "C:\\path\\to\\mock_audit_workbook.xlsx",
        "saved_workbook": "C:\\path\\to\\audit_results\\mock_audit_results.xlsx",
...
def _create_mock_scan_job_payload() -> Dict[str, Any]:
    """Generate a mock scan job payload."""
    mock_time = "2026-04-17T14:32:00"
    return {
        ...
        "error": "",
        "sbl_file": {
            "name": "mock_audit_workbook.xlsx",
            "path": "C:\\path\\to\\mock_audit_workbook.xlsx",
            "baseline_name": "BASELINE_MOCK_v3.2",
            "timestamp": mock_time,
            "sbl_model": "GEOINT_FD",
        },
        "output": {
...
def _create_mock_files() -> None:
    """Create mock artifacts used by the GUI on first launch."""
    from config import (
        AUDIT_CHECKLIST_DIR,
        AUDIT_RESULTS_DIR,
        JSON_CHECKLIST_DIR,
    ...
    from services.workbook_service import (
        _create_mock_audit_checklist_workbook,
        _create_mock_audit_results_workbook,
    )

    ensure_project_structure()

    # Remove deprecated example artifacts if they still exist.
    for obsolete in [
        JSON_REGISTRY_SNAPSHOTS_DIR / "example_registry_snapshot.json",
        JSON_CHECKLIST_DIR / "example_checklist.json",
        JSON_RESULTS_DIR / "example_result.json",
        JSON_SCAN_JOBS_DIR / "example_scan_job.json",
        AUDIT_CHECKLIST_DIR / "example_audit_checklist.xlsx",
        AUDIT_RESULTS_DIR / "example_audit_results.xlsx",
    ]:
        try:
    ...
    snapshot_path = JSON_REGISTRY_SNAPSHOTS_DIR / "mock_registry_snapshot.json"
    if not snapshot_path.exists():
        snapshot_path.write_text(json.dumps(_create_mock_registry_snapshot(), indent=2), encoding="utf-8")

    checklist_path = JSON_CHECKLIST_DIR / "mock_checklist.json"
    if not checklist_path.exists():
        checklist_path.write_text(json.dumps(_create_mock_checklist_json(), indent=2), encoding="utf-8")

    result_path = JSON_RESULTS_DIR / "mock_result.json"
    if not result_path.exists():
        result_path.write_text(json.dumps(_create_mock_result_json(), indent=2), encoding="utf-8")

    scan_job_path = JSON_SCAN_JOBS_DIR / "mock_scan_job.json"
    if not scan_job_path.exists():
        scan_job_path.write_text(json.dumps(_create_mock_scan_job_payload(), indent=2), encoding="utf-8")

    checklist_workbook = AUDIT_CHECKLIST_DIR / "mock_audit_checklist.xlsx"
    if not checklist_workbook.exists():
        _create_mock_audit_checklist_workbook(checklist_workbook)

    results_workbook = AUDIT_RESULTS_DIR / "mock_audit_results.xlsx"
    if not results_workbook.exists():
        _create_mock_audit_results_workbook(results_workbook)
