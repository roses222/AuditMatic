...
- prints launch commands (or launches app with `setup.bat --run`)

Minimum requirement for cloners:
...
Optional immediate launch after setup:
...
Refactor notes are in [docs/REFACTOR_GUIDE.md](docs/REFACTOR_GUIDE.md).
...
This launcher uses the lightweight quick-scan flow (no saved profiles/pipelines) and keeps terminal output visible, while still writing logs and report artifacts.
